...
public class NotificationService extends Service {
	...
	private void handleIntent(Intent intent) {
		...
		mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "ESD");
		mWakeLock.acquire();
	...
	private class PollTask extends AsyncTask<Void, Void, Void> {
		 ...
		 * you've pushed your  notification(s). This will: 
		 * 1) Kill the service so it doesn't waste precious resources 
		 * 2) Call onDestroy() which will release the wake lock, so the device  can go to sleep
		 * again and save precious battery.
		 * */
		@Override
		protected void onPostExecute(Void result) {
			// handle your data stopSelf();
